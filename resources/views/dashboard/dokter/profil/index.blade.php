@extends('layouts.dokter.dokter')

@section('title', 'Profil Dokter')

@section('content')

<div class="detail-container">

    {{-- Header Halaman --}}
    <div class="detail-header">
        <div>
            <h2>Profil Saya</h2>
            <span style="color: #777;">Informasi akun dan data pribadi Anda.</span>
        </div>
    </div>

    <div class="detail-grid">

        {{-- KARTU KIRI: FOTO & ROLE --}}
        <div class="info-card" style="text-align: center;">

            <div class="card-title">Foto Profil</div>

            <div style="display: flex; justify-content: center; margin: 2rem 0;">

                {{-- LOGIKA GAMBAR PROFIL --}}
                @php
                    // Ambil ID User yang sedang login
                    $idUser = auth()->user()->iduser;
                    
                    // Tentukan path gambar: public/images/users/{iduser}.jpg
                    $fotoPath = 'images/users/' . $idUser . '.jpg';
                @endphp

                @if(file_exists(public_path($fotoPath)))
                    {{-- JIKA GAMBAR ADA: Tampilkan Foto --}}
                    <img src="{{ asset($fotoPath) }}" alt="Foto Profil"
                        style="width: 120px; height: 120px; border-radius: 50%; object-fit: cover; border: 4px solid #e3f2fd;">
                @else
                    {{-- JIKA TIDAK ADA: Tampilkan Inisial (Default) --}}
                    <div class="avatar-circle" style="width: 100px; height: 100px; font-size: 3rem;">
                        <i class="fas fa-user-md"></i> {{-- ICON DOKTER --}}
                    </div>
                @endif

            </div>

            <h3 style="color: #3ea2c7; margin: 10px 0;">{{ $profil->nama_user }}</h3>
            <p style="color: #777; margin-bottom: 1.5rem;">{{ $profil->email }}</p>

            <span class="badge-status" style="font-size: 1rem; padding: 8px 20px;">
                {{ strtoupper($profil->nama_role) }}
            </span>
        </div>

        {{-- KARTU KANAN: DETAIL INFORMASI --}}
        <div class="info-card">
            <div class="card-title">
                <i class="fas fa-id-card"></i> Detail Informasi
            </div>

            <div class="info-row">
                <label>Nama Lengkap</label>
                <span>{{ $profil->nama_user }}</span>
            </div>

            <div class="info-row">
                <label>Email</label>
                <span>{{ $profil->email }}</span>
            </div>

            <div class="divider-dashed" style="margin: 1rem 0;"></div>

            <div class="info-row">
                <label>Jenis Kelamin</label>
                <span>
                    @if($profil->jenis_kelamin == 'L') Laki-laki
                    @elseif($profil->jenis_kelamin == 'P') Perempuan
                    @else - @endif
                </span>
            </div>

            <div class="info-row">
                <label>No. Handphone</label>
                <span>{{ $profil->no_hp ?? '-' }}</span>
            </div>

            <div class="info-row">
                <label>Bidang Dokter</label>
                <span>{{ $profil->bidang_dokter ?? '-' }}</span>
            </div>

            <div class="info-row">
                <label>Alamat Domisili</label>
                <span>{{ $profil->alamat ?? '-' }}</span>
            </div>
        </div>

    </div>

</div>

@endsection