...
@section('content')
    ...
    <x-resepsionis-form 
        ...
        action="{{ route('dashboard.resepsionis.registrasi-pet.store') }}" 
        backRoute="{{ route('dashboard.resepsionis.dashboard-resepsionis') }}"
    >
            ...
            <select id="idpemilik" name="idpemilik" class="form-control" required>
                ...
                @foreach ($pemilikList as $p)
                    <option value="{{ $p->idpemilik }}" {{ old('idpemilik') == $p->idpemilik ? 'selected' : '' }}>
                        {{ $p->user->nama ?? 'Tanpa Nama' }} ({{ $p->no_wa }})
                    </option>
                @endforeach
            </select>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            jenisSelect.addEventListener('change', function() {
                ...
                if (idJenis) {
                    ...
                    // Panggil API Resepsionis menggunakan helper URL Blade
                    // Pastikan route ini ada di routes/web.php!
                    const apiUrl = "{{ url('dashboard/resepsionis/api/get-ras') }}/" + idJenis;

                    fetch(apiUrl)
                        .then(response => {
                            if (!response.ok) throw new Error('Gagal mengambil data (Route 404/500)');
                        ...
                        .catch(err => {
                            ...
                            loadingText.style.display = 'none';
                            rasSelect.disabled = false;
                            rasSelect.innerHTML = '<option value="">Gagal memuat data (Cek Console)</option>';
                        });
                }
            });
        });
